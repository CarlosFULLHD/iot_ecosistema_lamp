...
def obtener_datos_globales():
    ...
    if not data:
        ...
        return pd.DataFrame()
    df = pd.DataFrame(data, columns=['fecha_creacion', 'UsuarioID', 'LampID', 'wh_por_hora'])
    df['fecha_creacion'] = pd.to_datetime(df['fecha_creacion'])
    return df
...
def crear_graficos(lamp_id, usuario_id):
    ...
    if not datos_lamp.empty:
        ...
        consumo_label.object = f"<div style='font-size: 20px; font-weight: bold;'>Consumo total en kWh: {total_kwh:.4f} kWh (Bs {costo_total:.2f})</div>"
    ...
    if not datos_globales.empty:
        ...
        user_data = pd.Series(user_counts).reset_index(name='value').rename(columns={'index': 'UsuarioID'})
        user_data['angle'] = user_data['value']/user_data['value'].sum() * 2*pi
        user_data['color'] = Category20c[len(user_data)]
        p8 = figure(height=350, title="Usuarios por Fecha de Creación", toolbar_location=None, tools="hover", tooltips="@UsuarioID: @value", x_range=(-0.5, 1.0))
        p8.wedge(x=0, y=1, radius=0.4, start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'), line_color="white", fill_color='color', legend_field='UsuarioID', source=user_data)
        ...
        lamp_data = pd.Series(lamp_counts).reset_index(name='value').rename(columns={'index': 'LampID'})
        lamp_data['angle'] = lamp_data['value']/lamp_data['value'].sum() * 2*pi
        lamp_data['color'] = Category20c[len(lamp_data)]
        p9 = figure(height=350, title="Lámparas por Fecha de Creación", toolbar_location=None, tools="hover", tooltips="@LampID: @value", x_range=(-0.5, 1.0))
        p9.wedge(x=0, y=1, radius=0.4, start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'), line_color="white", fill_color='color', legend_field='LampID', source=lamp_data)
        p9.axis.axis_label = None
        p9.axis.visible = False
        p9.grid.grid_line_color = None

        # Histograma de la frecuencia de las lámparas
        lamp_hist, lamp_edges = np.histogram(datos_globales['LampID'].astype('category').cat.codes, bins=15, density=True)
        p10 = figure(title="Frecuencia de las Lámparas", sizing_mode="stretch_width", height=250)
        p10.quad(top=lamp_hist, bottom=0, left=lamp_edges[:-1], right=lamp_edges[1:], fill_color="#b4b4dc")
        ...
        consumo_global_label = pn.pane.Markdown(f"<div style='font-size: 20px; font-weight: bold;'>Consumo total de todas las lámparas: {total_global_kwh:.4f} kWh (Bs {costo_global_total:.2f})</div>")
